...
class KLDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            kld_loss = - 0.5 * (1 + x_lv - y_lv 
                                   - torch.div(torch.pow(x_mu-y_mu, 2), torch.exp(y_lv))
                                   - torch.div(torch.exp(x_lv), torch.exp(y_lv))).sum(dim=1).mean()
            kld_loss *= sample_size

            elbo = loss + kld_loss
            #--------------------------------------------------------------------------------------------
            mu_dictionary = {f"x_mu{i}":x_mu[0,0,i].item() for i in range(x_mu.size(2))}
            lv_dictionary = {f"x_lv{i}":x_lv[0,0,i].item() for i in range(x_lv.size(2))}
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        for key in logging_outputs[0].keys():
            if "mu" in key or "lv" in key:
                metrics.log_scalar(
                    key, logging_outputs[0][key], 1 , round=5
                )        
