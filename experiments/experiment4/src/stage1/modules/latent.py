...
class LatentGeneratorNetwork(nn.Module):
    def __init__(self, x_dim, z_dim):
        ...
        self.z_dim = z_dim
        self.x_to_mu = Parameter(torch.randn((x_dim, 1, z_dim), dtype=torch.float32, device='cuda', requires_grad=True))
        self.x_to_lv = Parameter(torch.randn((x_dim, 1, z_dim), dtype=torch.float32, device='cuda', requires_grad=True))
        self.mult_attn_x_mu = MultiheadAttention(z_dim, 1, 
                                    kdim=x_dim, vdim=x_dim, encoder_decoder_attention=True)
        self.mult_attn_x_lv = MultiheadAttention(z_dim, 1, 
                                    kdim=x_dim, vdim=x_dim, encoder_decoder_attention=True)
