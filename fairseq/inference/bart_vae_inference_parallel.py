...
#from custom.simple_vae2.models.simple_vae_bart import SimpleVaeBART
from custom.abex1.models.cvae import CVaeBART as SimpleVaeBART
...
def main():
    ...
    parser.add_argument("--parallel", type=int, default=3,  action='store')
    ...
    if not os.path.isdir(data_path+"/source_parallel"):
        ...
        source_parallel = [os.path.join(data_path+"/source_parallel", i ) for i in os.listdir(data_path+'/source_parallel')]
        length = len(os.listdir(data_path+"/source_parallel"))
        assert config.parallel==length 
        print("make list is skipped because there are files already...")

    print("*--------Making Bart List----------*")
    barts = [SimpleVaeBART.from_pretrained(
                    'checkpoints/',
