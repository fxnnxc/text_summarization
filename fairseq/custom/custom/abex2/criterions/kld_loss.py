...
class KLDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    def __init__(
        ...
        self.count = 0
        self.writer = SummaryWriter("runs/"+task.args.user_dir+"runs_z")
    ...
    def forward(self, model, sample, reduce=True):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            #--------------------------------------------------------------------------------------------
        mu_dictionary = {f"x_mu{i}":x_mu[0,i,0].item() for i in range(x_mu.size(1))}
        lv_dictionary = {f"x_lv{i}":x_lv[0,i,0].item() for i in range(x_lv.size(1))}
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        for key in logging_outputs[-1].keys():
            if "mu" in key or "lv" in key:
                metrics.log_scalar(
                    key, logging_outputs[-1][key], 1 , round=3
                )
