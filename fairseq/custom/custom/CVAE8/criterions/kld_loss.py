...
writer = SummaryWriter("runs_z/MUVAR")
...
class KLDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            m_x, m_y, l_x, l_y = x_mu, y_mu, x_lv, y_lv
            # print(x_mu.size(1))
            writer.add_scalars("X_Mu", {str(i):x_mu[0,i,0].item() for i in range(x_mu.size(1)//2)}, self.count)
            writer.add_scalars("X_LV", {str(i):x_lv[0,i,0].item() for i in range(x_lv.size(1)//2)}, self.count)
            self.count += 1
