...
@register_criterion("annealing_loss")
class LabelSmoothedCrossEntropyCriterionAnnealing(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
        ...
        self.normalized_kld = normalized_kld
        self.annealing_R = annealing_R
        self.annealing_M = annealing_M
    ...
    def add_args(parser):
        ...
        parser.add_argument("--annealing-M", default=2, type=float)
        parser.add_argument("--annealing-R", default=0.5, type=float)
        parser.add_argument("--normalized-kld", default=False, type=bool)
        parser.add_argument("--total-number", default=14700, type=float)
    ...
    def forward(self, model, sample, update_num,  reduce=True):
        ...
        FREQ = self.total_number/self.annealing_M
        tau = ((update_num)%np.ceil(FREQ))/(FREQ)
        beta = min(1, tau)

        kld_weight = 1
        if self.normalized_kld:
            kld_weight = sample['nsentences']/287227.
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "kld_loss", kld_loss_sum / 1 / 1, 1, round=3
        )
