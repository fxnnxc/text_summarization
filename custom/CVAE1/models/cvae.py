...
class CVaeBART(BARTModel):
    ...
    def forward(
        ...
        x2 =encoder_out2['encoder_out'][0]

        xt = x.view(x.size(1), x.size(0), x.size(2))  # B x T x D
        xt = xt[:,:xt.size(1)//2,:]                   # B x T/2 x D
        xt = xt.mean(dim=1)                           # B x D
        mu = self.mlp_mu(xt)                          # B x D
        var = self.mlp_var(xt)                        # B x D
        z = self.reparameterize(mu, var)              # B x D
        xz = self.mult_attn(x, z, z)[0]               # T x B x D
        ...
        # print(encoder_out['encoder_embedding'][0].size())

        # print(encoder_out2['encoder_out'][0].size())
        # print(encoder_out2['encoder_padding_mask'][0].size())
        # print(encoder_out2['encoder_embedding'][0].size())

        # encoder_out['encoder_out'][0] = torch.cat([encoder_out['encoder_out'][0], encoder_out2['encoder_out'][0]], dim=0) 
        # encoder_out['encoder_padding_mask'][0] = torch.cat([encoder_out['encoder_padding_mask'][0], encoder_out2['encoder_padding_mask'][0]], dim=1) 
        # encoder_out['encoder_embedding'][0] = torch.cat([encoder_out['encoder_embedding'][0], encoder_out2['encoder_embedding'][0]], dim=1)
