...
class CVaeBART(BARTModel):
    ...
    def __init__(self, args, pretrained_bart):
        ...
        self.mult_attn_y_lv = MultiheadAttention(C, 1, 
                kdim=x_dim, vdim=x_dim, encoder_decoder_attention=True)
        self.mult_attn_xz = MultiheadAttention(x_dim, 1, encoder_decoder_attention=True)
        self.mult_attn_yz = MultiheadAttention(x_dim, 1, encoder_decoder_attention=True)
        ...
        self.XZtoV = nn.Linear(x_dim+C, len(self.decoder.dictionary), bias=False)
        self.XtoV  = nn.Linear(x_dim, len(self.decoder.dictionary), bias=False)
        self.ZtoV  = nn.Linear(C, len(self.decoder.dictionary), bias=False)
    ...
    def forward(
        ...
        x_mu = self.mlp_mu_x(x_mu)        # b x z x z
        ...
        y, extra = self.decoder(               # B x T x D
            prev_output_tokens,
        ...
        y_mu = y_mu.view(bsz, 1, -1)
        y_lv = y_lv.view(bsz, 1, -1)
        y_z = self.reparameterize(y_mu, y_lv)
        y_z = y_z.view(bsz, 1, -1)

        # 3. Abstractive Part Y
        y = y.reshape(-1, bsz, x_dim)
        y_z2 = y_z.view(1, bsz, -1)
        y_z2 = self.mlp_yz(y_z2)
        y = self.mult_attn_yz(y, y_z2, y_z2)[0]
        y = y.reshape(bsz,-1,x_dim)
        
        # =================
        # 6. Last Probability 
        # Extractive - Concat 
        x_z1 = y_z.view(bsz, 1, -1)
        x_z1 = x_z1.repeat(1, y.size(1), 1)
        xz2 = torch.cat([y, x_z1], dim=-1)  # B x T x (x_dim +Z) 
        xz3 = self.XZtoV(xz2)   # B x V

        # Extractive - Bag of words
        y_z2 = y_z.view(bsz, 1, -1)
        z = self.ZtoV(y_z2)

        # X to V
        y = self.XtoV(y)
        y += self.alpha * xz3 + (1-self.alpha) * z
