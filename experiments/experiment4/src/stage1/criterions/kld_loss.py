...
class KLDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            #--------------------------------------------------------------------------------------------
            mu_dictionary = {f"mu_x{i}":x_mu[0,0,i].item() for i in range(0, x_mu.size(2), x_mu.size(2)//10 ) }
            lv_dictionary = {f"lv_x{i}":x_lv[0,0,i].item() for i in range(0, x_lv.size(2), x_mu.size(2)//10 ) }
            mu_dictionary2 = {f"mu_y{i}":y_mu[0,0,i].item() for i in range(0, y_mu.size(2), y_mu.size(2)//10 ) }
            lv_dictionary2 = {f"lv_y{i}":y_lv[0,0,i].item() for i in range(0, y_lv.size(2), y_mu.size(2)//10 ) }
        ...
        logging_output.update(mu_dictionary2)
        logging_output.update(lv_dictionary2)
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        for key in logging_outputs[0].keys():
            if "mu" in key or "lv" in key:
                metrics.log_scalar(
                    key, logging_outputs[0][key], 1 , round=4
                )
