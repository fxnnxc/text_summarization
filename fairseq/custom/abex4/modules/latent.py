...
class LatentGeneratorNetworkC(nn.Module):
    def __init__(self, x_dim, z_dim):
        super(LatentGeneratorNetworkC, self).__init__()
        self.z_dim = z_dim
    ...
    def forward(self, x):
        ...
        x_lv = x_lv.view(bsz, z_dim, x_dim)                     # b x z x xdim
        x_mu = self.mlp_mu_x(x_mu)                              # b x z x 1
        x_lv = self.mlp_lv_x(x_lv)                              # b x z x 1
