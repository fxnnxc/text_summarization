...
class KLDLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    def __init__(
        ...
        self.count = 0
        self.writer = SummaryWriter("tensorboard/"+task.args.user_dir+"runs_z")
    @staticmethod
    def add_args(parser):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            # print(x_mu.size(1))
            self.writer.add_scalars("X_Mu", {str(i):x_mu[0,i,0].item() for i in range(x_mu.size(1)//2)}, self.count)
            self.writer.add_scalars("X_LV", {str(i):x_lv[0,i,0].item() for i in range(x_lv.size(1)//2)}, self.count)
            self.count += 1
            ...
            #kld_loss = - 0.5 * (1 + l_y - l_x 
            #                       - torch.div(torch.pow(m_y-m_x, 2), torch.exp(l_x))
            #                       - torch.div(torch.exp(l_y), torch.exp(l_x))).sum(dim=1).mean()
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "elbo", elbo_sum / sample_size / math.log(2), sample_size, round=3
        )

        metrics.log_scalar(
            "kld", kld_sum , sample_size, round=3
        )
