...
class SequenceGenerator(nn.Module):
    ...
    def _prefix_tokens(
        ...
        # # print("-------------------------")
        # # print(lprobs.size())
        # # print(prefix_toks.unsqueeze(-1).size())
        # # # print(prefix_mask.size())
        # # print("-------------------------")
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            # decode each model
            latent_vocab = model.get_latent_vocab(latent_z)
            if self.has_incremental_states():
                decoder_out = model.decoder.forward(
                    tokens,
                    encoder_out=encoder_out,
                    incremental_state=incremental_states[i],
                    features_only=model.new_linear
                )
            else:
                decoder_out = model.decoder.forward(tokens, encoder_out=encoder_out, features_only=model.new_linear)
            if model.new_linear:
                decoder_vocab = model.get_x_vocab(decoder_out[0])
                decoder_out = (decoder_vocab, decoder_out[1])
            ...
            if model.sep_softmax:
                alpha = model.alpha
                latent_out_tuple = (latent_vocab, None)
                probs_latent = model.get_normalized_probs(latent_out_tuple, log_probs=True, sample=None)
                probs += alpha*probs_latent

            # probs2 = model.get_normalized_probs(
