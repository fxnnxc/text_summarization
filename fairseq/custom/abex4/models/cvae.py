...
from ..modules.latent2 import LatentGeneratorNetwork
...
class CVaeBART(BARTModel):
    ...
    def __init__(self, args, pretrained_bart):
        ...
        # Moduels
        self.latent_x_layer = LatentGeneratorNetwork(self.x_dim, self.z_dim)
        self.latent_y_layer = LatentGeneratorNetwork(self.x_dim, self.z_dim)
    ...
    def forward(
        ...
        x, extra = self.decoder(         
            ...
            features_only=False, #features_only, #True,
            alignment_layer=alignment_layer,
            alignment_heads=alignment_heads,
            src_lengths=src_lengths,
            return_all_hiddens=return_all_hiddens,
        )
        y = extra['inner_states'][-1]
        y = y.view(y.size(1), y.size(0), -1)
        bsz = y.size(0) 
        ...
        # Combing y and latent Z
        # y_z_expand = self.mlp_yz(y_z)
        # y_z_expand = y_z_expand.reshape(1, bsz, -1)
        # y = self.mult_attn_yz(y, y_z_expand, y_z_expand)[0]
        # y = y.reshape(bsz,-1,self.x_dim)
        
        # P1 <Latent - Bag of words>
        # y_z_bag = y_z.view(bsz, 1, -1)
        # y1 = self.ZtoV(y_z_bag)
        
        # P2 <Latent- Concatenation> 
        y_z_cat = y_z.view(bsz, 1, -1)
        y_z_cat = y_z_cat.repeat(1, y.size(1), 1)
        y2 = torch.cat([y, y_z_cat], dim=-1) 
        y = self.XZtoV(y2)
        ...
        #y += self.alpha * y2 + (1-self.alpha) * y1
