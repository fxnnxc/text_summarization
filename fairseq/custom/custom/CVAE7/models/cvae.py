...
class CVaeBART(BARTModel):
    ...
    def __init__(self, args, pretrained_bart):
        ...
        C = 10 # Compression
        self.mlp_mu_x1 = nn.Sequential(
                    nn.Linear(args.encoder_embed_dim, C),
                    nn.ReLU(),
                    nn.Linear(C, C)
                )
        self.mlp_var_x1 = nn.Sequential(
                    nn.Linear(args.encoder_embed_dim, C),
                    nn.ReLU(),
                    nn.Linear(C, C)
                )
        self.mlp_mu_x2 = nn.Sequential(
                    nn.Linear(args.encoder_embed_dim, C),
                    nn.ReLU(),
                    nn.Linear(C, C)
                )
        self.mlp_var_x2 = nn.Sequential(
                    nn.Linear(args.encoder_embed_dim, C),
                    nn.ReLU(),
                    nn.Linear(C, C)
                )
        ...
        self.expand_z_x = nn.Linear(C, args.encoder_embed_dim)
        self.expand_z_y = nn.Linear(C, args.encoder_embed_dim)
        self.DtoV = nn.Linear(args.encoder_embed_dim, len(self.decoder.dictionary))
    ...
    def forward(
        ...
        xt1 = xt[:,:xt.size(1)//2, :].mean(dim=1)        # B x D
        xt2 = xt[:, xt.size(1)//2:, :].mean(dim=1)
        
        mu_x1, var_x1 = self.mlp_mu_x1(xt1), self.mlp_var_x1(xt1)
        mu_x2, var_x2 = self.mlp_mu_x2(xt2), self.mlp_var_x2(xt2)
        z_x1 = self.reparameterize(mu_x1, var_x1)
        z_x2 = self.reparameterize(mu_x2, var_x2)
        z_x = z_x1 + z_x2
        z_x = self.expand_z_x(z_x)               # B x D
        
        yt = y.view(y.size(1), y.size(0), y.size(2))
        yt = yt.mean(dim=1)
        mu_y, var_y = self.mlp_mu_y(yt), self.mlp_var_y(yt)
        z_y = self.reparameterize(mu_y, var_y)
        z_y = self.expand_z_y(z_y)
       

        xz = self.mult_attn(x, z_x, z_y)[0]
        encoder_out['encoder_out'][0] = xz

        x, extra = self.decoder(               # B x T x V
            prev_output_tokens,
        ...
        z_x = z_x.unsqueeze(dim=1)
        ztoD = self.DtoV(z_x)   # B x V
        x = ztoD + x
