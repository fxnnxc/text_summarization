...
class CVaeBART(BARTModel):
    ...
    def forward(
        ...
        mu_x = self.mlp_mu_x(xt)                          # B x D
        var_x = self.mlp_var_x(xt)                        # B x D
        z_x = self.reparameterize(mu_x, var_x)              # B x D
        xz = self.mult_attn(x, z_x, z_x)[0]               # T x B x D


        yt = y.view(y.size(1), y.size(0), y.size(2))
        yt = yt.mean(dim=1)
        mu_y = self.mlp_mu_y(yt)
        var_y = self.mlp_var_y(yt)
